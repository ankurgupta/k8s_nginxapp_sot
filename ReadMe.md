...
# Define source SOT
flux create source git k8s-nginxapp-sot \
 --url=https://github.com/ankurgupta/k8s_nginxapp_sot.git \
 --branch=main \
 --interval=30s \
 --export > ./deploy/flux_source.yaml

# Apply above source SOT using kustomization controller
 flux create kustomization k8s-nginxapp-sot \
 --source=k8s-nginxapp-sot  \
 --path="./deploy/" \
 --prune=true \
 --validation=client \
 --interval=30s  \
